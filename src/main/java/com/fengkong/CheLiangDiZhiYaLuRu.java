...
public class CheLiangDiZhiYaLuRu {
	...
	public void luRu(String name,int sendmoney,int aboutmoney) throws InterruptedException {
		...
		dr.findElement(By.name("amountd")).sendKeys(""+sendmoney);
		dr.findElement(By.name("termd")).sendKeys("12");//月
...
//		dr.findElement(By.name("registeredarea")).sendKeys("皇后大道");
		...
		dr.findElement(By.id("carid_1")).sendKeys("京NB8888");
		...
		dr.findElement(By.id("carmoney_1")).sendKeys(""+aboutmoney);
		// dr.findElement(By.id("ossupppp")).click();
		// dr.findElement(By.id("selectfiles")).click();

		 dr.findElement(By.name("savetype")).click(); //保存

//		dr.findElement(By.xpath("/html/body/div/div/div[2]/form/div/div[3]/div/button[2]")).click();// 提交
		try {
			Thread.sleep(4000);
		} catch (InterruptedException e) {
